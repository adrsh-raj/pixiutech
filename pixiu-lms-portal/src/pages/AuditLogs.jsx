...
import { ShieldCheck, UserCheck, Clock, Calendar, Search, RefreshCw, Trash2, Download, AlertCircle, KeyRound, Monitor, GraduationCap, Building } from 'lucide-react';
...
export default function AuditLogs() {
  ...
  const loadLogs = () => {
    try {
      ...
      // If empty and current user is present, seed the current active session
  ...
  const handleClearLogs = () => {
    if (window.confirm('Are you sure you want to clear all login activity logs?')) {
      localStorage.removeItem('pixiu_admin_logs');
      setLogs([]);
      toast.success('Login audit logs cleared successfully');
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Live chronological activity log of all logins (Students, Trainers, Partner Schools, and Administrators).
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Logins</p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Student Logins</p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Trainer Logins</p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Admin Logins</p>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search by student name, roll ID, or school..."
            value={searchQuery}
                  ...
                  <td colSpan="6" className="py-12 text-center text-slate-400">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-xs text-slate-400">
                        {searchQuery ? 'Try changing your search query or role filter.' : 'Logins from students, trainers, schools, and admins will appear here automatically.'}
                      </p>
                    </div>
                  </td>
                </tr>
